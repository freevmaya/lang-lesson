...
//            trace($a_query);
    ...
    static public function line($query, $args=null, $resType = RESULTTYPE, $cached=false) {
        if (!$cached || (($row = DB::getCache($query, $cacheKey)) === false)) { 
        	...
        	if (mysql_num_rows($result) < 1) {
        		...
        		$row=mysql_fetch_array($result, $resType);
        		mysql_free_result($result);
    ...
    static function one($query, $cached=false) {
        $row = DB::line($query, null, MYSQL_BOTH, $cached);
        return $row[0];
    }
    ...
    static public function asArray($query, $args=null, $cached=false) {
        if (!$cached || (($ret = DB::getCache($query, $cacheKey)) === false)) { 
            ...
            if ($cached) {
                DB::setCache($cacheKey, $ret);
                //trace($cacheKey.' '.$query);
            }
        }
    	return $ret;
    }
